...
import { ChromeService, NeonService, NotificationService } from '@/app/core';
...
export class PopupNoticeAuthComponent implements OnInit {
    ...
    public refuse() {
        this.chrome.getAuthorization().subscribe(res => {
            if (this.ruleCheck) {
                res[this.hostname] = {icon: this.iconSrc, title: this.title, status: this.ruleSelected };
                this.chrome.setAuthorization(res);
            }
            this.chrome.windowCallback({data: false, target: 'authorize_rejected'});
            window.close();
        });
    }
    public connect() {
        this.chrome.getAuthorization().subscribe(res => {
            ...
            if (this.isNext === 'transfer') {
                this.router.navigateByUrl(`/popup/notification/transfer?to_address=${this.paramsData.to_address}&asset_id=${this.paramsData.asset_id}&amount=${this.paramsData.amount}`);
            } else {
                this.chrome.windowCallback({data: true, target: 'authorized'});
                window.close();
            }
        });
    }
